...
<div>
  <navbar></navbar>
          ...
          <div class="row">     
          <div class="col-md-8">
          <h2><img src="/icons/draft.svg" style="height: 39px; width: 57px;">Reports- Project Quotation User Info</h2>
          </div>
          <div class="ml-auto" style="display:inherit">
          <a  :href="'/api/projectexcel/'+id+'/'+role_id+'/'+excelExportdata.user+'/'+excelExportdata.currency+'/'+excelExportdata.from_date+'/'+excelExportdata.to_date" download="project-report.xlsx" class="nav-link btn btn-primary" style="margin-right: 6px;color:white;height: 40px;
      ...
      getQuotation(val=null) {
        if(val==null) {
           var app = this
                     ...
                     this.$http.get("projectreport/"+localStorage.getItem('id')+"/"+localStorage.getItem('role_id') ).then((res) => {
                ...
                // console.log(app.excelExportdata);
                 ...
                 let formData = new FormData();
                    ...
                    formData.append('to_date',app.filter_arr.to_date),
                     this.$http.post("projectreport/"+localStorage.getItem('id')+"/"+localStorage.getItem('role_id'), formData ).then((res) => {
                  let users_id = localStorage.getItem('role_id');
                  this.gridData= res.data.data;
                }, () => {
                this.has_error = true
