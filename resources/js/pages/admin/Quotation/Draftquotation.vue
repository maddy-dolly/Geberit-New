...
<div>
  <navbar></navbar>
          ...
          <button class="btn btn-secondary btn-circle" data-toggle="modal" data-target="#myModal1" title="quotation info"><i class="fas fa-info"  @click="getRecord(props.entry.id)"></i></button>
      ...
      <tr v-for="(pre,index) in product" :key="index">
       ...
       <td>{{pre.total_cost}}</td>
         ...
         <a :href=userpdfpath+props.entry.id download="products.pdf" v-if="props.entry.status =='APPROVED'" class="btn btn-warning btn-circle" title="download user pdf"><i class="fas fa-file"></i></a>
          ...
          <router-link :to="{name:'viewquotation', params:{id:props.entry.id}}" class="btn btn-dark btn-circle" title="view quotation"><i class="fas fa-eye"></i></router-link>             
  ...
  methods:{
      ...
      getQuotation() {
          var app = this
               ...
               $.each(res.data.data.drafted, function(index,value) {
                  let assigned_to=[];
                  let approved_to=[];
               ...
               this.gridData= res.data.data.drafted;
